...
class ConnectionsController < ApplicationController
  ...
  def new
    ...
    payload = { "data": { "customer_id": current_user.customer_id, "return_connection_id": true,
                          "consent": { "scopes": %w[account_details transactions_details] },
                          "attempt": { "fetch_scopes": %w[accounts transactions] } } }
    response = RestClient::Request.execute(method: :post, url: url,
                                           headers: { accept: 'application/json',
                                                      'content-type' => 'application/json',
                                                      App_id: APP_ID, Secret: SECRET },
                                           payload: payload)
    redirect_to @connect_url = JSON.parse(response.body)['data']['connect_url']
  end
